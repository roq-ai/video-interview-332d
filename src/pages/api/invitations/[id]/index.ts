...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getInvitationById() {
    const data = await prisma.invitation.findFirst(convertQueryToPrismaUtil(req.query, 'invitation'));
    return res.status(200).json(data);
  }

  async function updateInvitationById() {
  ...
  async function deleteInvitationById() {
