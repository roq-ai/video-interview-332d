...
type ColumnType = ColumnDef<AnswerInterface, unknown>;
...
export function AnswerListPage(props: AnswerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AnswerInterface>>(
    () => `/answers?params=${JSON.stringify(params)}`,
    fetcher,
  );
    ...
    hasAccess('question', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/questions/view/${record.question?.id}`}>
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/answers/view/${record.id}`} passHref legacyBehavior>
                <Button
