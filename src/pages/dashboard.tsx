...
import { getAnswers } from 'apiSdk/answers';
...
function DashboardPage() {
  ...
  const answerStat = useSWR<PaginatedInterface<AnswerInterface>>(
    () => `/answers`,
    () => getAnswers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Answers',
      result: answerStat?.data?.totalCount,
      isLoading: answerStat.isLoading,
    },
  ];
